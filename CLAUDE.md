...
2. **Evidence on everything.** Findings follow `topoli/core/domain/finding.py`. Class A/B findings without `source.url` and `source.retrieved_at` fail validation → downgraded to D. Do not weaken this.
...
5. **No telemetry, no Topoli server calls.** Only official public sources. Local cache in `~/.topoli/cache/` (`topoli/core/adapters/cache.py`; tests run offline against recorded fixtures).
...
- Python code lives in the `topoli/` package (`topoli/core/…`, `topoli/countries/…`; see `docs/decisions/001-package-layout.md`); `skills/`, `ui/`, `i18n/`, `tests/`, `docs/` stay at the root. Adapters implement `topoli/core/adapters/base.py::Adapter`, live in `topoli/countries/<country>/<jurisdiction>/` and register in `topoli/core/adapters/registry.py`. Each adapter ships fixtures for ≥ 3 sites recorded with `topoli fixtures record` (`tests/fixtures/<adapter_id>/<slug>/`) and a nightly source-contract test marked `live` + `adapter("<id>")`.
