(function () {
  ...
  // map tiles: fetched from swisstopo only once the map scrolls into view (keeps first paint
  // free of external requests); offline → the static fallback image stays
  var M = D.map;
  var tilesEl = document.getElementById("tiles");
  function loadTiles() {
    if (!M || !tilesEl || navigator.onLine === false || tilesEl.getAttribute("data-loaded")) return;
    tilesEl.setAttribute("data-loaded", "1");
    var probe = new Image();
  ...
  if (tilesEl && "IntersectionObserver" in window) {
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) { if (en.isIntersecting) { loadTiles(); io.disconnect(); } });
    }, { rootMargin: "200px" });
    io.observe(tilesEl.parentNode);
  } else {
